...
(async()=>{
  ...
  for(const route of ['/','/shop','/robots.txt','/sitemap.xml','/shop/goool-athletics-modern-sport-performance-tee','/shop/goool-athletics-modern-sport-performance-tee?color=True+Royal']){
    ...
    if(route.includes('modern-sport')&&!t.includes('modern-open-a-20260925-v5'))throw Error('Missing fresh image revision '+route);
    ...
    report.pages.push({route,status:r.status,location:r.headers.get('location'),newRevision:route.includes('modern-sport')?t.includes('modern-open-a-20260925-v5'):undefined});
  }
  fs.mkdirSync(path.join(root,'live-optimizer'),{recursive:true});
  await Promise.all(records.map(async rec=>{
    const src='/products/'+rec.file+'?v=modern-open-a-20260925-v5';
